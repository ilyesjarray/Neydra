...
function fixAIPersonas() {
    ...
    // Escape backticks and $ in CSS so it's a safe literal string in TSX
    ...
    // The inner script needs to avoid using template literals with ${} in the TSX to prevent TS from trying to interpolate them at compile time.
    const scriptContent = `
const MODELS = [
...
setTimeout(() => {
    ...
    // Also escape scriptContent just in case
    const safeScriptContent = scriptContent.replace(/`/g, '\\`').replace(/\$/g, '\\$');
...
export function NeydraAIPersonas() {
    return (
        <div className="neydra-legacy-container flex-1 h-full w-full relative overflow-y-auto overflow-x-hidden bg-black text-white">
            ...
            ${body}
            <script dangerouslySetInnerHTML={{ __html: \`${safeScriptContent}\` }} />
        </div>
    );
}
`;

    fs.writeFileSync(path.join(__dirname, 'apps', 'web', 'src', 'components', 'sectors', 'Legacy', 'NeydraAIPersonas.tsx'), tsxCode.trim(), 'utf-8');
    console.log("Fixed NeydraAIPersonas.tsx");
}
