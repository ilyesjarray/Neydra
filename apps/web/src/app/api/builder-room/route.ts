// /api/builder-room — Next.js App Router: Multi-Agent Construction Pipeline
// Uses Groq for code generation, chains agents sequentially
// Returns files as base64 ZIP via SSE streaming
...
const BUILDER_MODEL = 'meta-llama/llama-4-maverick-17b-128e-instruct';
...
function createSimpleZip(files: { name: string; content: string }[]): string {
  const entries: any[] = [];
  let offset = 0;
...
Keep files minimal but complete. Maximum 6 files.`;
          ...
          try {
            ...
            sendEvent({ type: 'error', message: 'Failed to parse build plan.' });
            controller.close();
            return;
          }
...
- Write clean, professional code.`;
          ...
          controller.enqueue(encoder.encode('data: ' + JSON.stringify({ type: 'error', message: 'Internal build error: ' + err.message }) + '\n\n'));
        }
        controller.close();
      },
    });
