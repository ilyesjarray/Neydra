...
export class ReportGenerator {
    static generatePortfolioReport(assets: any[], totalValue: number) {
        ...
        doc.text('SECURED BY NEYDRA ORACLE PROTOCOL. ALL DATA VERIFIED IN REAL-TIME.', 15, doc.internal.pageSize.height - 10);

        doc.save(`Neydra_Wealth_Report_${Date.now()}.pdf`);
    }
    ...
    static generateIntelligenceBrief(intel: any, type: string = 'SIGNAL_VERIFICATION') {
        ...
        doc.save(`Neydra_Briefing_${Date.now()}.pdf`);
    }
}
