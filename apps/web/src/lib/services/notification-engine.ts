...
class NotificationEngine {
    ...
    public async dispatch(alert: Omit<NeydraAlert, 'id' | 'timestamp'>) {
        const fullAlert: NeydraAlert = {
            ...alert,
    ...
    private playAudioCue(level: AlertLevel) {
        ...
        console.log(`[NeydraAudio] PLAY_PULSE_FREQ_${frequencies[level]}Hz_DUR_200ms`);
    }

    public generateDawnBriefing(): Omit<NeydraAlert, 'id' | 'timestamp'> {
        return {
            type: 'DAWN_BRIEFING',
            level: 2,
    ...
    public generateBlackSwanAlert(event: string): Omit<NeydraAlert, 'id' | 'timestamp'> {
        return {
            type: 'BLACK_SWAN',
            level: 5,
...
export const neydraNotifications = NotificationEngine.getInstance();
