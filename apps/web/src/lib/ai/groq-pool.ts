...
export function getGroqKey(): string {
    if (GROQ_KEYS.length === 0) {
        throw new Error('NEYDRA_ERROR: No Groq API keys configured.');
    }
...
export async function callGroq(
    ...
    throw lastError || new Error('NEYDRA_ERROR: All Groq API keys exhausted.');
}
...
export async function callWhisper(audioBlob: Blob, model: string = 'whisper-large-v3-turbo'): Promise<string> {
    ...
    throw lastError || new Error('NEYDRA_ERROR: Whisper transcription failed on all keys.');
}
