...
/**
 * Neydra PDF Intelligence Engine
 ...
 */
export async function exportToNeydraPDF({
    elementId,
    filename,
    watermark = 'NEYDRA ELITE INTELLIGENCE // RESTRICTED ACCESS',
    orientation = 'landscape'
}: PDFExportOptions): Promise<void> {
    const targetElement = document.getElementById(elementId);

    if (!targetElement) {
        console.error(`Neydra PDF Engine: Element with ID ${elementId} not found.`);
    ...
    try {
        ...
        pdf.setFontSize(8);
        pdf.setTextColor(255, 0, 0); // Neydra Red
        pdf.text(
        ...
        console.error('Neydra PDF Generation failed:', error);
        throw error;
    } finally {
