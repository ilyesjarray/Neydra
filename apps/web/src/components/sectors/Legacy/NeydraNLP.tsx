...
export function NeydraNLP() {
    return (
        ...
        * {
        ...
        /* Sentiment Container (Replaces Chart) */
        .sentiment-container {
            flex: 1;
            background: #000;
        ...
        /* --- RIGHT SIDEBAR: VOLATILITY & KEYWORDS --- */
        .sidebar-right {
            grid-row: 2 / 3;
            grid-column: 3 / 4;
            display: flex;
        ...
        /* --- TARGET LOGIC STYLES (Red Protocol Skinned) --- */
        ...
        /* Red Protocol Specific Overrides */
        .bullish {
            color: #fff;
        ...
        <div className="modal-content">
            ...
            <div className="modal-body">
                ...
                <div className="step-container">
                    ...
                    <div className="step-description">
                        Download the NEYDRA NLP Package below. Extract the <code>.rar</code> file, then double-click
                        <code>ultra_nlp_backend.exe</code> to start the engine.
                        <div className="requirements-list">
                            <h4>Instructions</h4>
                            <ul>
                                <li>Extract the <code>.rar</code> file to a folder</li>
                                <li>Double-click <code>ultra_nlp_backend.exe</code> to run</li>
                ...
                <div className="step-container">
                    ...
                    <div className="step-description">
                        Once the tunnel page shows "status": "ONLINE", paste the URL into the connection panel and click
                        Connect.
                    </div>
                </div>
    ...
    <div className="viewport">
        <div className="header-section">
            <h1>NEYDRA <span>GLOBAL NLP</span></h1>
    ...
    {/* <script>
        ...
        // --- DOM ELEMENTS ---
        const elSentiment = document.getElementById('sentimentScore');
        const elRegime = document.getElementById('marketRegime');
        const elNews = document.getElementById('newsFeed');
        const elVol = document.getElementById('volValue');
        const elVolAlert = document.getElementById('volAlert');
        const elKeywords = document.getElementById('keywordsList');
            ...
            fetch(`${API_URL}/`)
                ...
                .then(data => {
                    if (data.status === "ONLINE" || data.name) {
                        log("NLP ENGINE CONNECTION ESTABLISHED", "suc");
        ...
        // --- DOWNLOAD HANDLED VIA DIRECT .RAR LINK ---
        // The backend .exe is packaged in /downloads/neydra-nlp-package.rar
        // No inline script generation needed.

        /* REMOVED: Inline Python script was here. Now served as compiled .exe in .rar package. * /
        ...
        async function fetchUltraData() {
            if (!isRunning || !API_URL) return;

            try {
                const response = await fetch(`${API_URL}/ultra_data`);
                if (!response.ok) throw new Error("API Error");
                const data = await response.json();

                // 1. UPDATE SENTIMENT METER (Center Top)
                elSentiment.innerText = data.global_sentiment > 0 ? "+" + data.global_sentiment : data.global_sentiment;
                elRegime.innerText = data.market_regime;

                // Color Logic: Red Protocol
                if (data.global_sentiment > 0) {
                    elSentiment.style.color = "#fff"; // Bright White/Red for Bull
                    elSentiment.style.textShadow = "0 0 20px #ff0000";
                    elRegime.style.color = "var(--primary)";
                } else if (data.global_sentiment < 0) {
                    elSentiment.style.color = "var(--dim-red)"; // Dark Red for Bear
                ...
                data.latest_headlines.slice(0, 20).forEach(news => {
                    const item = document.createElement('div');
                    item.className = 'news-item';

                    let scoreClass = "neutral";
                    if (news.score > 0) scoreClass = "bullish";
                    if (news.score < 0) scoreClass = "bearish";

                    allKeywords.push(...news.keywords);

                    item.innerHTML = `
                        <div className="news-meta">
                            <span>${news.datetime} | ${news.source}</span>
                            <span className="${scoreClass}">SENT: ${news.score}</span>
                        </div>
                        <div className="news-title">
                            <span style={{ "color": "var(--dim-red)" }}>[${news.ticker}]</span> ${news.title}
                        </div>
                    `;
                    elNews.appendChild(item);
                });

                // 3. UPDATE VOLATILITY (Right Sidebar Top)
                elVol.innerText = data.volatility_forecast + "%";
                if (data.volatility_forecast > 5.0) {
                    elVol.style.color = "#fff";
                    elVolAlert.innerText = "VOLATILITY EXPANSION IMMINENT.";
                    elVolAlert.className = "vol-alert critical";
                    sfx('alert');
                } else if (data.volatility_forecast < -5.0) {
                    elVol.style.color = "var(--dim-red)";
                    elVolAlert.innerText = "VOLATILITY CONTRACTION.";
                    elVolAlert.className = "vol-alert";
                } else {
                    elVol.style.color = "var(--primary)";
                    elVolAlert.innerText = "MARKET STABILITY NOMINAL.";
                    elVolAlert.className = "vol-alert";
                }

                // 4. UPDATE KEYWORDS (Right Sidebar Bottom)
                elKeywords.innerHTML = "";
                const counts = {};
                allKeywords.forEach(x => { counts[x] = (counts[x] || 0) + 1; });
                const sortedKw = Object.keys(counts).sort((a, b) => counts[b] - counts[a]).slice(0, 15);

                sortedKw.forEach(k => {
                    const span = document.createElement('span');
                    span.className = 'kw-tag';
                    span.innerText = `${k} (${counts[k]})`;
                    elKeywords.appendChild(span);
                });

                // 5. Generators
                generateHex();
                if (Math.random() > 0.95) sfx('tick');

            } catch (error) {
                if (Math.random() > 0.95) log("NLP CONNECTION UNSTABLE", "err");
            }
        }
        ...
        function toggleEngine() {
            ...
            if (isRunning) {
                ...
                log("ULTRA NLP PROTOCOL ENGAGED", "suc");
                sfx('engage');
                ...
                // Start the loop
                setInterval(fetchUltraData, 5000);
                fetchUltraData(); // Initial fetch
            }
        }
