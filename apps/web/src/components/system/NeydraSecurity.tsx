...
/**
 * ======================================================
 * NEYDRA™ FULL SECURITY SHIELD v2.1
 ...
 * ======================================================
 */
export function NeydraSecurity() {
    ...
    useEffect(() => {
        if (isBypassed) {
            ...
            const cssEl = document.getElementById('neydra-security-css');
        ...
        if (!currentHost || allowedDomains.indexOf(currentHost) === -1) {
            document.documentElement.innerHTML = 
                ...
                '</div></body>';
            setTimeout(() => { window.location.href = 'https://neydra.vercel.app'; }, 3000);
            return;
        }
    ...
    useEffect(() => {
        ...
        const checkVersion = async () => {
            ...
            try {
                ...
                const localVersion = localStorage.getItem('neydra_version');
                
                if (!localVersion) {
                    // First time load, just set it
                    localStorage.setItem('neydra_version', liveVersion);
